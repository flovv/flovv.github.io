...
```{r, echo=FALSE, message=FALSE}
ggplot(trips, aes(mon, dt, group=mon )) + geom_boxplot() + theme_economist()
```
...
```{r, echo=FALSE, message=FALSE}
ggplot(working, aes(y=wt, x=mon)) +geom_boxplot()  + ylab("time spend at work in hours") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + theme_economist()
